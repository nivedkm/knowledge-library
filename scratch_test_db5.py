...
with Session(engine) as session:
    ...
    res1 = session.execute(stmt1).all()
    print("Without ORDER BY:", len(res1))
    
    stmt2 = stmt1.order_by(semantic_distance.asc())
    res2 = session.execute(stmt2).all()
    print("With ORDER BY semantic_distance ASC:", len(res2))
